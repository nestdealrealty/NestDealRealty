...
const icons = {
  ...
  /* Ready To Move — completed building with checkmark */
  completed: (
    <svg width="40" height="40" viewBox="0 0 48 48" fill="none">
...
export default function ExploreGandhinagar() {
  ...
  const currentStats = [
    { num: dynamicStats.total.toString(), suffix: '', label: 'Total Projects', sub: 'Across Gandhinagar', icon: icons.totalProjects },
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: '1300px', margin: '0 auto' }}>
        ...
        <style>{`
          ...
          @media (max-width: 1024px) {
            .eg-stats-row-top { grid-template-columns: repeat(3, 1fr); max-width: 100%; }
            .eg-stats-row-bottom { grid-template-columns: repeat(2, 1fr); }
          }
          @media (max-width: 600px) {
            .eg-stats-row-top { grid-template-columns: 1fr; }
            .eg-stats-row-bottom { grid-template-columns: 1fr; }
          }
        `}</style>
        {/* Top Row - 3 items centered */}
        <div className="eg-stats-row-top">
          {currentStats.slice(0, 3).map((s, i) => (
            <StatCard key={i} stat={s} visible={visible} delay={0.35 + i * 0.08} />
          ))}
        </div>
        {/* Bottom Row - 4 items */}
        <div className="eg-stats-row-bottom">
